...
Once these settings have been specified, the datasets can be generated by running the `CREATE_DATASET.sh` bash script.  For example, the dataset for solving the Poisson equation on varying domains can be created via:
```console
