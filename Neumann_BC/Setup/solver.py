...
def FEM_solver(resolution, mesh_resolution, mesh_ID, data_ID, use_hires=False):
    ...
    #square_V = FiniteElement("CG", square_mesh, 1)

    V = FunctionSpace(mesh, V)
    R = FunctionSpace(mesh, R)
    ...
    # Specify homogeneous Neumann boundary condition
    ...
    A = (inner(a*I_mat*nabla_grad(u), nabla_grad(v)) + c*v + u*d)*dx
    L = f*v*dx + g*v*ds
    ...
    # Recover solution 'u' and scaling 'c'
    (u, c) = w.split()
