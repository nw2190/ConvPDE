...
def gen_mesh(resolution, vertex_min, vertex_max, mesh_resolution, mesh_directory, ID):
    ...
    reindexing = np.argsort(percent_angle)
    percent_angle = percent_angle[reindexing]
    ...
    # Sort radius values
    #radii_list_1 = np.sort(normalized_radii[:int(normalized_radii.size/2)])
    #radii_list_2 = np.flip(np.sort(normalized_radii[int(normalized_radii.size/2):]))
    #normalized_radii = np.sort(normalized_radii)
    #normalized_radii = np.concatenate([radii_list_1, radii_list_2])

    # REFINE BOUNDARY
    tmp_percent_angle = np.copy(percent_angle)
    tmp_normalized_radii = np.copy(normalized_radii)
    percent_angle = np.zeros([2*N])
    normalized_radii = np.zeros([2*N])
    for i in range(0,2*N):
        if np.mod(i,2) == 0:
            percent_angle[i] = tmp_percent_angle[int(i/2)]
            normalized_radii[i] = tmp_normalized_radii[int(i/2)]
        elif i == 2*N-1:
            percent_angle[i] = 0.5 * ( tmp_percent_angle[int((i-1)/2)] + (1.0 + tmp_percent_angle[0]) )
            normalized_radii[i] = 0.5 * ( tmp_normalized_radii[int((i-1)/2)] + tmp_normalized_radii[0] )
        else:
            percent_angle[i] = 0.5 * ( tmp_percent_angle[int((i-1)/2)] + tmp_percent_angle[int((i+1)/2)] )
            normalized_radii[i] = 0.5 * ( tmp_normalized_radii[int((i-1)/2)] + tmp_normalized_radii[int((i+1)/2)] )
