...
# Recover values and alpha mask corresponding to data image
def read_data(data_ID, data_directory, USE_HIRES=False, **keyword_parameters):
    if USE_HIRES:
...
def plot_prediction(ID, vals, scale, soln_dir, mesh_dir, Model=0, CV=1, Rm_Outliers=False, Filter=True, Plot_Error=True, SHOW_PLOT=True, default_res=128, refine=1, slice_plot=False, use_hires=False, save_plots=False, view_elev=30, view_angle=0, error_view_elev=30, error_view_angle=0, PLOT_ALL=False):
    ...
    if slice_plot:
        mesh = mesh[0:int(default_res/2),0:int(default_res/2)]
    ...
    pred_vals, plot_X, plot_Y = preprocesser(pred, Rm_Outliers=False, Filter=FILTER, refine=refine)
    std_vals, plot_X, plot_Y = preprocesser(stds, Rm_Outliers=False, Filter=True, refine=refine)
    ...
    # Show std values if plot is sliced
    if slice_plot:
        #tri_ax1.plot_trisurf(plot_X_flat,plot_Y_flat, pred_plus_two_stds_flat, cmap=cmap, linewidth=linewidth, antialiased=True, alpha=0.1)
        tri_ax1.plot_trisurf(plot_X_flat,plot_Y_flat, pred_plus_stds_flat, cmap=cmap, linewidth=linewidth, antialiased=True, alpha=0.1) 
        #tri_ax1.plot_trisurf(plot_X_flat,plot_Y_flat, pred_minus_two_stds_flat, cmap=cmap, linewidth=linewidth, antialiased=True, alpha=0.1)
        tri_ax1.plot_trisurf(plot_X_flat,plot_Y_flat, pred_minus_stds_flat, cmap=cmap, linewidth=linewidth, antialiased=True, alpha=0.1) 
    ...
    if Plot_Error:
        ...
        # CLIP ERROR BOUNDS
        #threshold = 1.1*np.max([np.abs(diff_min),np.abs(diff_max)])
        threshold = 0.4*np.max([np.abs(diff_min),np.abs(diff_max)])
        two_std_vals_flat = np.clip(two_std_vals_flat, -threshold, threshold)
        neg_two_std_vals_flat = np.clip(neg_two_std_vals_flat, -threshold, threshold)
...
def plot_data(ID):
    ...
    FILTER = True
    refine = 1
    data_vals, plot_X, plot_Y = preprocesser(data, Rm_Outliers=False, Filter=FILTER, refine=refine)
    ...
    for i in range(0,R):
        for j in range(0,R):
            if mesh[i,j] > 0.0:
                plot_X_flat.append(mesh_x_vals[i])
                plot_Y_flat.append(mesh_y_vals[j])
                data_vals_flat.append(data_vals[i,j])
                
    tri_fig = plt.figure()
    tri_ax1 = tri_fig.add_subplot(111, projection='3d')
    tri_ax1.plot_trisurf(plot_X_flat,plot_Y_flat, data_vals_flat, cmap=cmap, linewidth=linewidth, antialiased=True)
    plt.show()
