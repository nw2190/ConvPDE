...
def preprocess_data(start_index, data_count, data_dir, mesh_dir, soln_dir, RESCALE=True, use_hires=False):
    ...
    for i in range(start_index, start_index + data_count):
        if use_hires:
            ...
            hires_coeff = np.load(data_dir + 'hires_coeff_' + str(i) + '.npy')
            hires_coeff[hires_out_of_domain] = 0.0
            ...
            np.save(data_dir + 'hires_coeff_' + str(i) + '.npy', hires_coeff)            
            ...
            coeff = np.load(data_dir + 'coeff_' + str(i) + '.npy')
            coeff[out_of_domain] = 0.0            
            ...
            np.save(data_dir + 'coeff_' + str(i) + '.npy', coeff)            
