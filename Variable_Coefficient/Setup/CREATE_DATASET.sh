...
if [ $failed -eq 0 ]; then
    python Generate_Samples.py --gen_stiffness
    if [ $? -ne 0 ]; then
        failed=1
        fail_file="Generate_Samples.py --gen_stiffness"
    fi
fi
if [ $failed -eq 0 ]; then
...
if [ $failed -eq 0 ]; then
    python Convert_Samples.py --gen_stiffness
    if [ $? -ne 0 ]; then
        failed=1
        fail_file="Convert_Samples.py --gen_stiffness"
    fi
fi
if [ $failed -eq 0 ]; then
