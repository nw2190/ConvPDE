    ...
    with tf.Session(graph=graph) as sess:
        ...
        for _ in range(0,1):
            ...
            for n in range(0, batches):
                ...
                #scaling_maxes = np.abs(np.amax(data_batch, axis=(1,2,3))[:,np.newaxis,np.newaxis,np.newaxis])
                #scaling_mins  = np.abs(np.amax(data_batch, axis=(1,2,3))[:,np.newaxis,np.newaxis,np.newaxis])
                #scaling_factors = np.amax(data_batch, axis=(1,2,3))[:,np.newaxis,np.newaxis,np.newaxis]
                #scaling_maxes = np.abs(np.amax(data_batch, axis=(1,2,3)))
                #scaling_mins  = np.abs(np.amax(data_batch, axis=(1,2,3)))
                #scaling_factors = np.amax(np.concatenate([scaling_maxes[:,np.newaxis],scaling_mins[:,np.newaxis]], axis=1), axis=1)[:,np.newaxis,np.newaxis,np.newaxis]
