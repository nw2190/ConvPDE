...
def encoder(self, x, training=True, reuse=None, name=None):
    ...
    h3 = inception_v3(h2, 72, stride=1, batch_norm=False, training=training, reuse=reuse, name='e_incept_1')

    # [None, 16, 16, 64] --> [None, 8, 8, 64]
    h4 = conv2d_layer(h3, 72, kernel_size=3, batch_norm=False, regularize=self.regularize, training=training, reuse=reuse, name='e_conv_3')#, coordconv=self.coordconv)
    ...
    chans = 600 if self.use_kl else 300
...
def decoder(self, z, training=True, reuse=None, name=None):
    ...
    h = inception_v3(h, 300, batch_norm=self.use_bn, regularize=self.regularize, training=training, reuse=reuse, name='d_incept_0')

    h = tf.layers.dropout(h, rate=self.dropout_rate, training=training)


    if self.coordconv:
        h = conv2d_layer(h, 300, kernel_size=2, batch_norm=False, regularize=self.regularize, training=training, reuse=reuse, name='d_conv_0', coordconv=self.coordconv)
