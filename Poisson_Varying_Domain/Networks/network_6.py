...
def encoder(self, x, training=True, reuse=None, name=None):
    ...
    chans = 128 if self.use_kl else 64
    h4 = conv2d_layer(h3, chans, kernel_size=3, batch_norm=self.use_bn, regularize=self.regularize,
    ...
    if self.use_inception:
        chans = 256 if self.use_kl else 128
        h5 = inception_v3(h4,chans, batch_norm=self.use_bn, regularize=self.regularize,
...
def decoder(self, z, training=True, reuse=None, name=None):
    ...
    # h6 ~ [None, 4, 4, 256]
    h = h6
    h = inception_v3(h, 256, batch_norm=self.use_bn, regularize=self.regularize, training=training,
                     reuse=reuse, name='d_incept_0')
    ...
    h = transpose_conv2d_layer(h, 128, kernel_size=2, batch_norm=self.use_bn, regularize=self.regularize,
                               stride=stride, training=training, reuse=reuse, name='d_tconv_0')
    if self.interpolate:
        h = upsample(h, 4*2)
    ...
    h = transpose_conv2d_layer(h, 64, kernel_size=2, batch_norm=self.use_bn, regularize=self.regularize,
                               stride=stride, training=training, reuse=reuse, name='d_tconv_1')
    if self.interpolate:
        h = upsample(h, 4*2*2)
    ...
    h = transpose_inception_v3(h, 32, stride=stride, batch_norm=self.use_bn, regularize=self.regularize,
    ...
    h = transpose_conv2d_layer(h, 32, kernel_size=3, batch_norm=self.use_bn, regularize=self.regularize,
                               stride=1, training=training, reuse=reuse, name='d_tconv_2_1')
    h = transpose_conv2d_layer(h, 32, kernel_size=3, batch_norm=self.use_bn, regularize=self.regularize,
                               stride=stride, training=training, reuse=reuse, name='d_tconv_2_2')
    if self.interpolate:
        h = upsample(h, 4*2*2*2*2)

    #h = tf.concat([h, h1],3)

    # [None, 64, 64, 16] --> [None, 128, 128, 1]
    s = transpose_conv2d_layer(h, 1, kernel_size=6, batch_norm=False, stride=2, activation=None,
                               add_bias=False, training=training, reuse=reuse, name='d_tconv_3_s')
    h = transpose_conv2d_layer(h, 1, kernel_size=6, batch_norm=False, stride=2, activation=None,
...
def evaluate_model(self, data, reuse=None, training=True, suffix=None):
    ...
    if self.use_kl:
        ...
        for h in z_kl:
            ...
            h_list.append(tf.nn.leaky_relu(self.sampleGaussian(m, log_s, name='latent_sample')))
